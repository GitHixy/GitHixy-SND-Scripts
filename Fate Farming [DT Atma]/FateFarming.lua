...
description: >-
  ...
  - Displays atma progress in status messages (gems count, moving to fate)
plugin_dependencies:
- Lifestream
- vnavmesh
- TextAdvance
...
********************************************************************************
    ...
    -> 3.1.0    By GitHixy.
                ...
                New metadata options: "Enable Atma Farming?" and "Target Atma per Zone".
                Atma progress now displayed in gem count and "moving to fate" messages.
                Added AtmaData table with all 6 Dawntrail zones, item IDs, and aetheryte mappings.
                New helper functions for atma tracking, progress checking, and zone switching.
                Smart initialization: teleports to first incomplete zone on script start.
                Full SND v2 API compliance for inventory checks and zone detection.
    -> 3.0.9    By Allison.
...
AtmaData = {
    zones = {
        ...
        { name = "Living Memory", zoneId = 1192, itemId = 47749, itemName = "Orpiment Demiatma", aetheryteName = "Leynode Mnemo" }
    }
}
...
function Repair()
    ...
    if SelfRepair then
        ...
        if needsRepair.Count > 0 then
            ...
            -- Close aethernet menu if open
            if Addons.GetAddon("TelepotTown").Ready then
                yield("/callback TelepotTown false -1")
                return
            end
            
            if distanceToMender > 60 then
                -- We're far from mender, use aethernet to get to Hawkers' Alley
                yield("/li Hawkers' Alley")
                yield("/wait 1")
...
if TargetAtmaPerZone == nil or TargetAtmaPerZone < 1 then
